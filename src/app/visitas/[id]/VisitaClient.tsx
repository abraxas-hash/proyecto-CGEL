          ...
          <div className="relative max-w-4xl w-full flex flex-col items-center" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-[#050505] border border-white/20 p-2 rounded-2xl shadow-2xl w-full overflow-hidden">
              ...
              <div className="p-5 flex justify-between items-center bg-white/5">
                <div>
                  <p className="text-slate-800 dark:text-white font-bold text-xl">{visita.nombre_completo}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass-panel rounded-3xl p-8 relative overflow-hidden bg-gradient-to-br from-green-500/[0.05] to-transparent">
            <div className="flex flex-col items-center text-center mb-8">
              ...
              <h2 className="text-2xl font-black text-slate-800 dark:text-white capitalize leading-tight">{visita.nombre_completo?.toLowerCase()}</h2>
            ...
            <div className="space-y-4">
              <div className="bg-white/5 p-4 rounded-2xl border border-slate-700">
                ...
                <p className="text-slate-800 dark:text-white font-bold">{visita.empresa || 'PARTICULAR'}</p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/5 p-4 rounded-2xl border border-slate-700 text-center">
                  <Clock className="w-4 h-4 text-green-400 mx-auto mb-2" />
                  <p className="text-[10px] text-gray-500 uppercase font-bold">Ingreso</p>
                  <p className="text-lg font-mono text-slate-800 dark:text-white mt-1">{visita.hora_ingreso?.slice(0,5)}</p>
                </div>
                <div className="bg-white/5 p-4 rounded-2xl border border-slate-700 text-center">
                  <Clock className="w-4 h-4 text-orange-400 mx-auto mb-2" />
                  <p className="text-[10px] text-gray-500 uppercase font-bold">Salida</p>
                  <p className="text-lg font-mono text-slate-800 dark:text-white mt-1">{visita.hora_salida?.slice(0,5) || '--:--'}</p>
                </div>
              </div>
              ...
              <div className="bg-white/5 p-4 rounded-2xl border border-slate-700">
                ...
                <p className="text-sm text-slate-800 dark:text-white font-bold mb-1">Visitó a: {visita.referencia_visita}</p>
                <p className="text-xs text-slate-500 dark:text-gray-400 leading-relaxed italic">"{visita.motivo_visita}"</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-3xl p-6">
            <h3 className="text-xs font-black text-slate-500 dark:text-gray-400 uppercase mb-6 tracking-[0.2em]">Auditoría de Protocolo</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-slate-700">
                  <div className="flex items-center gap-3">
                    <span className="text-gray-500">{item.icon}</span>
                    <span className="text-xs text-slate-600 dark:text-gray-300 font-medium">{item.label}</span>
                  </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="glass-panel rounded-3xl p-8 h-full flex flex-col">
            <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-4">
              ...
              <h2 className="text-xl font-bold text-slate-800 dark:text-white uppercase tracking-tight">Récord de Ingresos</h2>
            </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
              {historial?.map((hist: any) => (
                <Link 
                  ...
                  className="block group bg-white/5 hover:bg-white/10 border border-slate-700 p-4 rounded-2xl transition-all"
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-slate-800 dark:text-white font-mono">{hist.fecha}</span>
                    <span className="text-[9px] font-bold text-slate-800 dark:text-white group-hover:underline">VER DETALLE</span>
                  </div>
                  <p className="text-[10px] text-gray-500 leading-tight">
                    Visitó a: <span className="text-slate-600 dark:text-gray-300 font-bold">{hist.referencia_visita?.slice(0,20)}</span>
                  </p>
        ...
        <div className="lg:col-span-4 glass-panel rounded-3xl p-6 bg-gradient-to-b from-transparent to-green-900/5">
          <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
            ...
            <h2 className="text-xl font-bold text-slate-800 dark:text-white uppercase tracking-tight">Cámaras y Evidencia</h2>
          </div>
              ...
              {mergedEvidencias.map((foto: any, i: number) => (
                ...
                >
                  <div className="relative aspect-video w-full overflow-hidden">
                    ...
                    />
                    <div className="absolute top-3 right-3 bg-black/80 backdrop-blur-md px-3 py-1 rounded text-[9px] font-black text-slate-800 dark:text-white border border-white/10 uppercase tracking-widest">
                      {foto.tipo_evidencia}
                    </div>
                  </div>
                  <div className="p-4 border-t border-slate-700 flex justify-between items-center">
                    <p className="text-[10px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-widest">{foto.etiqueta}</p>
