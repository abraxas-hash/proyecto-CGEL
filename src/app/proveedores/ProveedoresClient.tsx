...
export default function ProveedoresClient({ initialProveedores }: { initialProveedores: any[] }) {
  ...
  const uniqueDates = useMemo(() => {
    const dates = Array.from(new Set(initialProveedores?.map(p => p.fecha) || []));
    ...
    <main className="glass-panel rounded-2xl p-2 sm:p-4 w-full min-h-[600px] flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row gap-6 flex-1 h-full overflow-hidden">
        ...
        <aside className="w-full md:w-56 shrink-0 flex md:flex-col overflow-x-auto md:overflow-y-auto no-scrollbar gap-2 pb-2 md:pb-0 md:pr-2 border-b md:border-b-0 md:border-r border-white/5">
          <span className="hidden md:block text-[9px] text-gray-600 font-black uppercase tracking-widest mb-3 px-2">Calendario de Cargas</span>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 p-3 rounded-xl transition-all shrink-0 md:shrink ${
                  isActive 
                  ? 'bg-blue-500/10 border border-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.05)]' 
                  : 'bg-white/[0.02] border border-transparent hover:bg-white/[0.05]'
                }`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${isActive ? 'bg-blue-500 border-blue-500 text-black' : 'bg-white/5 border-white/10 text-gray-500'}`}>
