...
export default function PoliticasPage() {
  const sections = [
    ...
    },
    {
      ...
      icon: Users,
      content: 'Diseñado exclusivamente para el personal de Supervisión de CGEL Security y la Gerencia de Operaciones/SOMA de Sonepar Perú. El acceso está restringido mediante roles jerárquicos para proteger la privacidad de los datos personales (DNI, SCTR) de conductores y contratistas.',
  ...
  const responsibilities = [
    ...
    { role: 'Supervisores CGEL', task: 'Auditoría diaria, verificación de alertas y mantenimiento del sistema.' },
    { role: 'Gerencia Sonepar', task: 'Consulta de métricas, toma de decisiones SSOMA y gestión de infraestructura.' },
      ...
      <main className="max-w-6xl mx-auto">
        <div className="mb-12 text-center">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            ...
            <span className="text-blue-400 font-bold"> CGEL-CONTROL</span>.
          </p>
        </div>
          ...
          {sections.map((section, idx) => (
            <div key={idx} className="glass-panel p-8 rounded-3xl relative overflow-hidden group hover:border-blue-500/30 transition-all">
              <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity">
                <section.icon className="w-32 h-32 text-white" />
              </div>
        ...
        <div className="glass-panel rounded-3xl p-8 border-l-4 border-l-orange-500 bg-orange-500/5">
          <div className="flex items-center gap-3 mb-8">
            <Scale className="w-6 h-6 text-orange-400" />
            <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Matriz de Responsabilidades</h2>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {responsibilities.map((res, idx) => (
              <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-5">
                <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest mb-2">{res.role}</p>
                <p className="text-sm text-gray-300 font-medium leading-snug">{res.task}</p>
              </div>
            ))}
          </div>
        </div>
